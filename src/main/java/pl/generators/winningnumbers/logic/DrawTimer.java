...
class DrawTimer {
    ...
    public LocalDateTime generateDrawDate() {
        return LocalDateTime.now(clock).getDayOfWeek().equals(DayOfWeek.SATURDAY) &&
                ...
                LocalDateTime.now(clock).toLocalDate().with(TemporalAdjusters.nextOrSame(DayOfWeek.SATURDAY))
                                                      .atTime(HOUR_OF_DRAW, MINUTE_OF_DRAW) :
                LocalDateTime.now(clock).toLocalDate().with(TemporalAdjusters.next(DayOfWeek.SATURDAY))
                                                      .atTime(HOUR_OF_DRAW, MINUTE_OF_DRAW);
    }
